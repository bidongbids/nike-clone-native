// lib/rbac.ts
// NOTE: Admin nav items are included but admin screens are skipped by default.
// 'to' paths kept as-is — update to React Navigation screen names if admin is added later.

import { LayoutDashboard, Package, ShoppingBag, Users, BarChart2, Shield } from 'lucide-react-native' // swapped
...
export interface AdminNavItem {
  to: string          // update to RN screen name if admin screens are added
  label: string
  icon: ComponentType<{ size?: number; color?: string }> // typed for lucide-react-native
